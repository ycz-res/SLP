...
# **变换操作**
# 1. 取负，实现y轴翻转
train_loss_values = [-val for val in train_loss_values]
val_loss_values = [-val for val in val_loss_values]

# 2. 整体上移，让最小值大于0
min_train = min(train_loss_values)
min_val = min(val_loss_values)
offset = abs(min(min_train, min_val)) + 1  # 确保最低点在 1 以上

train_loss_values = [val + offset for val in train_loss_values]
val_loss_values = [val + offset for val in val_loss_values]
